              ...
              {filteredRooms.map((room) => (
                <div key={room.id} className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
                  <div className="relative h-48">
                    <Image
                      src={typeof room.image === 'string' ? room.image : "/placeholder.svg"}
                      alt={room.name}
                      fill
                    ...
                    {showResults && (
                      <div className="absolute top-4 right-4 bg-green-500 text-white px-2 py-1 rounded text-xs font-medium">
                        {roomAvailabilities[room.id] || 0} {t("bookingsPage.available")}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="p-6">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h3 className="text-xl font-cormorant font-semibold text-slate-800 mb-2">{room.name}</h3>
                        <p className="text-slate-600 font-alegreya text-sm mb-2">{room.description}</p>
                      </div>
                      <button
                        onClick={() => toggleCompareRoom(room.id)}
                        ...
                        title={t("bookingsPage.comparison.addToCompare")}
                      >
                    ...
                    <div className="flex items-center gap-4 mb-4 text-sm text-slate-600">
                      <span className="flex items-center gap-1">
                        ...
                        {room.maxGuests} {t("bookingsPage.guests")}
                      </span>
                      <span className="flex items-center gap-1">
                    ...
                    <div className="flex items-center gap-2 mb-4">
                      {room.wifi && <span className="flex items-center gap-1"><Wifi className="h-4 w-4 text-slate-400" /><span className="text-xs text-slate-400">WiFi</span></span>}
                      {room.airConditioning && <span className="flex items-center gap-1"><Snowflake className="h-4 w-4 text-slate-400" /><span className="text-xs text-slate-400">AC</span></span>}
                      {room.tv && <span className="flex items-center gap-1"><Tv className="h-4 w-4 text-slate-400" /><span className="text-xs text-slate-400">TV</span></span>}
                      {room.balcony && <span className="flex items-center gap-1"><Building className="h-4 w-4 text-slate-400" /><span className="text-xs text-slate-400">Balcony</span></span>}
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-2xl font-cormorant font-bold text-[#8B4B5C]">{room.price}€</span>
                          <span className="text-xs text-slate-500 font-alegreya">
                            {t("bookingsPage.perNight")}
                          </span>
                        </div>
                        {nights > 0 && (
                          <div className="text-sm font-medium text-slate-700 font-alegreya">
                            {t("bookingsPage.totalPrice")
                              .replace("{price}", (room.price * nights).toFixed(0))
                              .replace("{nights}", nights.toString())}
                          </div>
                        )}
